...
async function main() {
  ...
  for (const cat of categorias) {
    ...
    // Se a categoria não tem tipos, vamos criar um tipo 'Geral'
    if (tipos.length === 0) {
    ...
    for (let i = 1; i <= 4; i++) {
      const tipo = tipos[(i - 1) % tipos.length]; // Distribui entre os tipos disponíveis
      ...
      if (isDefective || (defectCount < targetDefects && cat.id === categorias[categorias.length-1].id && i > 1)) {
        condicao = 'MANUTENCAO'; // Or 'DEFEITO' depending on the system, schema says string, I will use MANUTENCAO and recebeuComDefeito = true
        comDefeito = true;
        defectCount++;
      }
  ...
  // Se por acaso não tiver criado 3 defeitos no loop aleatório, cria mais uns para completar
